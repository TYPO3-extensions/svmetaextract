...
******************************************************************************/
	...
	function parseXMP2simpleArray( $XMP_array ) {
        // Create a string to receive the output html
        $simpleArray = array();
        ...
        if ( $XMP_array !== FALSE ) {
			...
			foreach ($RDF_Contents as $RDF_Item)   {
                    ...
                    if ( ( $RDF_Item['tag'] == 'rdf:Description' ) AND is_array($RDF_Item['attributes']))  {
                        // Item is a rdf:Description tag.

                        // Cycle through each of the attributes for this tag, looking
                        foreach( $RDF_Item['attributes'] as $key => $val ) {
                        	list ($ns, $name) = explode(':', $key, 2);
                        	if ($ns!='xmlns' AND $ns!='rdf') {
                        		$simpleArray[$key] = $val;
                        	}
                        }
                    }
		        ...
		        if ( ( $RDF_Item['tag'] == 'rdf:Description' ) && ( array_key_exists( 'children', $RDF_Item ) ) )   {
	                ...
	                if ( array_key_exists( 'children', $RDF_Item ) )  {
                        ...
                        foreach( $RDF_Item['children'] as $child_item )   {
                                ...
                                $simpleArray[$tag_caption] = $value_str;
                        }
	                }
                        ...
                        if ( array_key_exists( 'tag', $child_item ) )
                        {
                                ...
                                switch ( $child_item[ 'tag' ] )
                                {
                                        ...
                                        case "rdf:Alt":
                                                ...
                                                $output_str = implode("\n", interpret_RDF_collection( $child_item ));
                                                break;

                                        case "rdf:Bag":
                                                # $output_str .= "Unordered List:\n";
                                                $output_str = implode(', ', interpret_RDF_collection( $child_item ));
                                                break;
                ...
                foreach( $item['children'] as $list_item )
                {
                             ...
                             ( preg_match ( "rdf:_\d+", $list_item['tag'] ) == 1 ) )
                        {
                                ...
                                if ( array_key_exists( 'value', $list_item ) )
                                {
                                        ...
                                        $output[] = get_RDF_field_html_value( $list_item );
                                }
...
/*
echo '<pre>';
foreach ($GLOBALS[ 'XMP_tag_captions' ] as $key => $desc) {
	$key = strtolower($key);
	echo "case '$key':\n";
	echo "	# $"."this->out['fields']['$key'] = $value;\n";
	echo "break;\n";
}
echo '</pre>';
*/
